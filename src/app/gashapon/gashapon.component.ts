...
export class GashaponComponent {
  ...
  pullLever(isCursedGold: boolean): void {
    if (isCursedGold) {
      this.gaschaResult = this.randNum.getRandomNumber(1, 100);
    } else {
      this.goldAmount = this.goldAmount < 0 ? 0 : this.goldAmount;
      this.gaschaResult = this.goldAmount + this.randNum.getRandomNumber(1, 6) > 100 ? 100 :
        this.goldAmount + this.randNum.getRandomNumber(1, 6);
    }
